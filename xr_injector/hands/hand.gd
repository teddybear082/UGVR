...
## Hack to use same pose settings as XR Tools
@onready var XRToolsHandPoseSettings = preload("res://xr_injector/hands/poses/hand_pose_settings.gd").new()
@onready var XRHelpers = preload("res://xr_injector/xr_helpers.gd").new()
...
## Default hand pose
@export var default_pose: Resource: set = set_default_pose
...
static func find_instance(node : Node):
	var XRHelpers = load("res://xr_injector/xr_helpers.gd").new()
	return XRHelpers.find_xr_child(
...
static func find_left(node : Node):
	var XRHelpers = load("res://xr_injector/xr_helpers.gd").new()
	return XRHelpers.find_xr_child(
...
## [XRToolsHand] assuming the node is a sibling of the [XROrigin3D].
static func find_right(node : Node):
	var XRHelpers = load("res://xr_injector/xr_helpers.gd").new()
	return XRHelpers.find_xr_child(
...
func set_default_pose(pose) -> void:
	default_pose = pose
	if is_inside_tree():
		_update_pose()
...
## Add a pose override
func add_pose_override(who : Node, priority : int, settings) -> void:
...
func _update_pose() -> void:
	...
	var pose_settings = default_pose
...
func _insert_pose_override(who : Node, priority : int, settings) -> void:
